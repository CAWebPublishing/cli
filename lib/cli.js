...
module.exports = function cli() {
	...
	// Add original commands from wp-env cli.
	yargs = wpenv_cli();
	
	// Add init command.
	yargs.command(
		'init',
		'Generates the .wp-env.json and docker-compose.override.yml file used and downloads CAWeb Plugins and Theme repositories.',
		( args ) => {
			args.option( 'exclude-phpmyadmin', {
				type: 'boolean',
				describe:
					'Exclude phpMyAdmin Docker Service.',
				default: false,
			} );
			args.option( 'exclude-plugins', {
				type: 'boolean',
				describe:
					'Exclude CAWeb Plugins in WordPress installation.',
				default: false,
			} );
			args.option( 'exclude-theme', {
				type: 'boolean',
				describe:
					'Exclude CAWeb Theme in WordPress installation.',
				default: false,
			} );
			args.option( 'exclude-init', {
				type: 'boolean',
				describe:
					'Disables generation of CAWeb Publishing WordPress configurations.',
				default: false,
			} );
		},
